...
----------------------------------------------------------------------------------------
MacKay에 따르면, 특정 모형(M_i)이 사실이라고 가정하고 데이터 (D)가 주어졌을 때 모수 W 사후 확률은 이러하다. 
$$ P(w|D, M_i) = \frac{P(D|w, M_i)P(w|M_i)}{P(D|M_i)} $$
정규화 상수인 $P(D|M_i)$는 대부분 unknown 상태로 넘억도 되지만, 모델 선택에서는 매우 중요하다. \
이는, 모델(i)에 대한 증거이기 때문이다. 
쉽게 말하면, 그 모델에서 데이터가 관찰될 확률이기 때문에 정규화 상수이면서도 모델 증거로서, 모델에 대한 선호도를 나타낸다. 
증거는 적분으로 정의되는데, 모수 w를 적분해서 없애기 때문에 증거를 marginal likelihoo라라고도 부른다. 
그런데, 여기서 적분을 하기에는 굉장히 어렵다. 그러므로 근사법을 사용하여 게산하는데, 보통 라플라스 근사를 사용한다. 
...
모델 증거(model evidence)의 식은 다음과 같다. (편의상 $M_i$ 생략)
$$ p(D)=\int p(D|\theta)p(\theta)d\theta (4.136) $$
근데 위에서 우린 정규화 상수 이미 구했지! 
$$ p(D)=\int p(D|\theta)p(\theta)d\theta = f(z_0)\frac{(2\pi)^{M/2}}{|A|^{1/2}} $$


여기에 로그를 취하면, 식 4.137이 나오게 되는데, 
<img width="298" alt="image" src="https://user-images.githubusercontent.com/71582504/220565663-f348fcaa-896e-4612-bb2c-ce7c00b7552c.png">
첫번째 항은 해당 모델이 data에 fitting이 얼마나 되었는지 이며, 오른쪽 세 항은 오컴 인자로서 모델의 복잡도에 대해 불이익을 주는 것이다. 
...
사후 분포의 가우시안 표현을 찾는 것이 목표인 만큼 일반 형태로 적을 수 있는 가우시안 1) 사전 분포에서부터 시작해보자. 
$$ p(w) = N(w|m_0, S_0)  (식 4.140) $$

2)w에 대한 사후분포
$$ p(w|t) \propto p(w)p(t|w) (식 4.141) $$

3)log 가능도 함수
양변에 로그를 취하고 식 4.140의 사전 분포와 식 4.89의 가능도 함수(참고로 식 4.89는 로그를 취한 것)를 대입해 넣으면 아래와 같다. 

$$ lnp(w|t) = -\frac{1}{2}(w - m_0)^T S_0^{-1}(w-m_0) + \sum_{n=1}^N (t_n ln y_n + (1-t_n) ln (1-y_n)) $$
왼쪽은 가우시안 형태고, 오른쪽은 가우시안 형태가 아니다.  \
즉, $ lnp(w|t) $가 가우시안 형태가 아니므로, 가우시안 형태로 만들어야 한다. \

여기서 
$$ y_n = \sigma(w^T\phi_n) $$ 
이는 232p(4.3.2)에서 언급된다. $ \phi_n $는 feature vector 임...

로그 가능도 함수를 미분해보면, 
$$ -\sum_{n=1}^N \frac{t_n(y_n)(1-y_n)}{y_n} \phi_n + \frac{-(1-t_n)y_n(1-y_n)}{1-y_n} \phi_n $$
$$ = -\sum_{n=1}^N(t_n - t_n y_n - y_n + t_n y_n)\phi_n $$
$$ = -\sum_{n=1}^N(y_n - t_n) \phi_n $$

우선, 사후 분포의 가우시안 근사를 구하기 위해서는 사후 분포를 최대화하여 최대 사후분포 해인 $w_MAP$를 구해야 한다. \
최대 사후 분포 해를 바탕으로 가우시안 분포의 평균값을 정의할 수 있다. \
공분산은 음의 로그 가능도 함수의 이차 미분값 행렬의 역으로 주어진다. \
<img width="392" alt="image" src="https://user-images.githubusercontent.com/71582504/220573683-d028afe1-71d1-4e07-af84-ae6fe6563474.png">

이제 사후 분포의 가우시안 근삿값은 다음과 같다. 
$$ q(w) = N(w|w_MAP, S_N) $$
이제 실제로 예측을 시행하기 위해서 이 분포에 대해 주변화를 해보면 됨

## 4.5.2 예측 분포
새 특징 벡터가 주어졌을 때, 클래스 c1에  대한 예측 분포는 사후 분포에 대해서 주변화를 시행해서 구할 수 있다. \
이 사후 분포는 가우시안 분포로 근사가 가능하다. 
